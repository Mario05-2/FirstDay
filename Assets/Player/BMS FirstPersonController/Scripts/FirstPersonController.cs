...
public class FirstPersonController : MonoBehaviour
{
    ...
    private void Update(){
        ...
        if (isLook){
            ...
            float mouseX = inputManager.RightStickInput.x * 10 * mouseSensitivity * Time.deltaTime;
            float mouseY = inputManager.RightStickInput.y * 10 * mouseSensitivity * Time.deltaTime;
            ...
            if (isSliding){
                playerCamera.transform.localRotation = Quaternion.Lerp(playerCamera.transform.localRotation,
                    Quaternion.Euler(yVelocity - slideTiltAngle, 0f, 0f), Time.deltaTime * 10f);
            }
            else{
        ...
        if (canSlide && isSprinting && inputManager.ButtonEast.Held() && isGrounded){    
            isSliding = true;
    ...
    private void HandleMovement(){
        ...
        moveInput.x = inputManager.LeftStickInput.x;
        moveInput.y = inputManager.LeftStickInput.y;
        
        //isSprinting = canSprint && Input.GetKey(KeyCode.LeftShift) && moveInput.y > 0.1f && isGrounded && !isCrouching && !isSliding;
        
        isSprinting = canSprint && inputManager.LeftStickPress.Held() && moveInput.y > 0.1f && isGrounded && !isCrouching && !isSliding;
        ...
        if (isGrounded || coyoteTimer > 0f){
            ...
            if (canJump && inputManager.ButtonSouth.Pressed() && !isSliding){
